...
print("=" * 80)
print("🏋️ ULTIMATE 'HIGHEST LEVEL' DEEP TRAINING (MASSIVE ARCHITECTURE) (MoRTH / NHAI SIH2026)")
print("=" * 80)
...
# ==============================================================================
print("\n--- [1/4] Training Model M1 (Upgraded): Vision Distress Net [512, 256, 128] ---")
...
X_m1_list = [rdd_train["features"], kaggle["features"], crack["features"], hn["features"]]
y_m1_list = [rdd_train["labels"], kaggle["labels"], crack["labels"], np.zeros(len(hn["features"]), dtype=np.int64)]

geo_rdd = rdd_train["bboxes"]
geo_extra = np.tile([0.45, 0.55, 0.20, 0.15], (len(kaggle["labels"]) + len(crack["labels"]) + len(hn["features"]), 1)).astype(np.float32)
...
m1_model = VisionDistressNet(in_dim=64, num_classes=5)
epochs_m1 = 25
batch_size = 256
...
# ==============================================================================
print("\n--- [2/4] Training Model M4 (Upgraded): 100Hz IMU Shock Classifier [256, 128] ---")
...
m4_model = IMUShockClassifier(in_features=feats_imu_tr.shape[1], hidden_dims=[256, 128], num_classes=4, lr=0.002)
...
# ==============================================================================
print("\n--- [3/4] Training Model M_PCI (Upgraded): ASTM D6433 PCI Regressor [256, 128, 64] ---")
...
pci_model = PCIRegressorNet(in_features=12, hidden_dims=[256, 128, 64], lr=0.001)
...
# ==============================================================================
print("\n--- [4/4] Training Model M_DEGRADE (Upgraded): Monsoon Forecaster [256, 128, 64] ---")
...
life_model = PavementDeteriorationForecaster(in_features=5, hidden_dims=[256, 128, 64], lr=0.001)
